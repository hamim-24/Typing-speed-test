...
int T = 0; // for total words from file

//Taking string from file
void input_from_file() {
    char a[MAX_WORD_LENGTH];
    while (scanf("%s", a) != EOF) {
        if (T >= MAX_STR_SIZE) { // overflowing
            fprintf(stderr, "Too many words in the file! Maximum allowed is %d.\n", MAX_STR_SIZE);
            exit(1);
        }
        strncpy(ALL_WORD[T++], a, MAX_WORD_LENGTH - 1);
        ALL_WORD[T - 1][MAX_WORD_LENGTH - 1] = '\0'; // Ensure null-termination
    }

    if (T == 0) { // if the file hasn't any word
        endwin();
